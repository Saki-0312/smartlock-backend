...
const mqtt = require('mqtt');  // 新增 MQTT 客户端
...
function subscribeBafaMqtt(uid) {
    ...
    bafaMqttClient.on('connect', () => {
        ...
        // 订阅 door002 主题（接收 ESP32 上报的消息）
        bafaMqttClient.subscribe('door002', (err) => {
            if (err) {
    ...
    bafaMqttClient.on('message', (topic, message) => {
        ...
        // 存储到数据库（注意：这里需要知道是哪个 uid）
        // 由于我们是用特定 uid 连接的，消息都属于这个 uid
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        if (errcode) {
            return res.json({ code: -1, message: '微信登录失败' });
        }
...
app.post('/api/messages', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        // 可选：标记为已读
        // await dbRun('UPDATE device_messages SET is_read = 1 WHERE uid = ? AND topic = ? AND id <= ?', 
        //             [uid, topic, messages.length ? messages[messages.length-1].id : 0]);
...
app.post('/api/query', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        res.json({
            code: 0,
            data: latest || { msg: 'unknown', timestamp: 0 }
        });
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`   POST /api/query        - 查询状态`);
